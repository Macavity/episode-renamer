...
class ConfigTest {
    ...
    fun `loadConfig should return correct Config`() {
        ...
        val expectedConfig = Config(shows = listOf(Show(name = "Test Show", aliases = listOf("Alias 1", "Alias 2"), seasonDirectories = false)))
    ...
    fun `loadConfig should return correct Config with seasonDirectories`() {
        ...
        val expectedConfig = Config(
            shows = listOf(
                Show(
                    name = "Test Show", seasonDirectories = true, aliases = listOf("Alias 1", "Alias 2")
                )
            )
        )
        every { configLoader.loadConfig() } returns expectedConfig
